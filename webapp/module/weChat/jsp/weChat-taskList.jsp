...
<body>
	<div id="section_container">
		<section id="form_section" data-role="section" class="active">
			<header>
				<div class="tabbar">
					<a class="tab active" data-role="tab" href="#article_unreceive" data-toggle="article">未接受</a> <a class="tab" data-role="tab" href="#article_unfinish" data-toggle="article">进行中</a> <a class="tab" data-role="tab" href="#article_finish" data-toggle="article">已完成</a>
				</div>
			</header>
			<article data-role="article" id="article_unreceive" data-scroll="verticle" class="active" style="top: 45px; bottom: 0px;">
				<div class="scroller">
					<ul class="listitem">
						<c:forEach items="${taskVo.unReceiveTaskList }" var="task">
							<li><a href="${path }/weChat/taskView?id=${task.id}&employeeId=${employee.id}">
			...
			<article data-role="article" id="article_unfinish" data-scroll="verticle" style="top: 45px; bottom: 0px;">
				<div class="scroller">
					<ul class="listitem">
			...
			<article data-role="article" id="article_finish" data-scroll="verticle" style="top: 45px; bottom: 0px;">
				<div class="scroller">
					<ul class="listitem">
						<c:forEach items="${taskVo.finishTaskList }" var="task">
							<li><a href="${path }/weChat/taskView?id=${task.id}&employeeId=${employee.id}">
									<div class="img appimg" style="padding-left: 5px;">
										<img alt="" src="${task.publisher.headimgurl}" style="position: absolute; top: 0; left: 0; width: 64px; border-radius: 50%;" />
									</div> <a href="${path }/weChat/publishEvaluation?publisherId=${employee.id}&receiverId=${task.publisher.id}&taskId=${task.id}" class="button ricon ">去评价</a>
									<div class="text" style="padding-left: 10px; color: #000;">
										${task.title } <small> 发布人：${task.publisher.name }<br>  任务类型：${task.taskType.typeName}<br /> 工期：${task.durTime}<br />发布时间：<fmt:formatDate value="${task.createTime }" pattern="yyyy-MM-dd HH:mm" /> <br />
										</small>
